...
def next_move(turn,current_moves,color):
    """Return the next move from opening based on current_moves

    Args:
        turn (int): turn
        current_moves (string list): moves
        color (int): color
    ...
    if turn == 1 and color == "white":
        # First turn, juste return a random first move from all opening
        opn = openings[random_between_a_and_min_bc(0,len(white_openings),len(black_openings))]
        return opn[0]
    for opening in openings:
        if len(current_moves) >= len(opening): # More move than opening
            continue
        
        # There is the first opening we can use
        i = 0
